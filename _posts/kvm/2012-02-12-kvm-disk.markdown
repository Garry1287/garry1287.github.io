...
блок (блоковое устройство)
	...
	в формате raw или host_device (хз but this will fail for block devices - you need to use "-O host_device". Sadly that wasn't documented so we've submitted a patch to fix that
					[http://bitcube.co.uk/content/converting-kvm-images-logical-volumes](http://bitcube.co.uk/content/converting-kvm-images-logical-volumes)   Converting files using "qemu-img convert" onto logical volumes (or any
